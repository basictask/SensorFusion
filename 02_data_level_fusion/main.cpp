...
// clear && cmake . && make && ./main "data/mono/aloe1.png" "data/mono/aloe_disp1.png"
// mono: aloe, art, baby, cloth, cones, flowerpots, lampshade, midd, monopoly, plastic, rocks, wood
// rgbd: adirondack, backpack, bicycle, cable, classroom, mask, motorcycle, piano, pipes, playroom, playtable, vintage

// Parameters set by user
int window_size = 5;									// kernel size for all filtering operations
vector<int> sigmas_to_try = {3, 5, 7, 11};		 		// iterate over there window sizes for optimization
float sigma = -1;										// distribution parameter for the gaussian curve
...
const string param_file = "bestparams_sig.csv";			// best performing params get printed here
...
int main(int argc, char** argv) 
{
	...
	for(int idx = 0; idx < sigmas_to_try.size(); idx++) // Iterate over given window sizes
	{
		...
		sigma = sigmas_to_try.at(idx);  			// Set the sigma size for the current iteration  

		cout << "------------------ Parameters -------------------" << endl;
		cout << "              progress = " << (float)idx / sigmas_to_try.size() * 100 << "%" << endl;
		...
		cout << "            sigma list = [" << sigmas_to_try.at(0);
		for(int i = 1; i < sigmas_to_try.size(); i++)
			cout << ", " << sigmas_to_try.at(i); 
		cout << "]" << endl;
...
void GetBestMetric()
{
	...
	cout << "         Optimal sigma = " << sigmas_to_try.at(best_i) << endl;
	...
	fout << image_name << ";" <<  sigmas_to_try.at(best_i) << ";" << best_metric << ";" << best_method << endl;
	fout.close();	
}
