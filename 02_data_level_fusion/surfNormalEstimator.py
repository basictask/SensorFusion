...
checkfilenames = ['aloe','art','baby','cloth','cones','flowerpots','lampshade','midd','monopoly','plastic','rocks','wood']
...
if(__name__ == '__main__'):
    ...
    file_to_process = 'aloe1_pointcloud_iterative_size=1_kernel=7.xyz'
    
    if(len(sys.argv) >= 1):
        # Case for when files were passed through comand line arguments
        for filename in sys.argv[1:]:
            print('Creating oriented point cloud for: ', filename, '...')
            process_file(directory, filename, k, keep_points)
            print('Creating oriented point cloud for: ', 'Done')
    ...
    if(file_to_process == ''): ##~~ Warning: this will eat your processing power ~~##
        for filename in os.listdir(directory): # Iterate over all files in the directory and assign normals to the points
            if(filename.endswith(".xyz") and check_in(filename, checkfilenames)):
                print('processing: ' + filename)
                process_file(directory, filename, k, keep_points)
                print('Done.')
                # break # just in case to not have massive exec time on our hands
    else:
        # This is just for the case when the filename was manually input
